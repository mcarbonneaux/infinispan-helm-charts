...
| ----- | ----------- | ------- | ---------------------- |
| `images.server` | FQN of the Infinispan Server image to deploy. | `quay.io/infinispan/server:16.1` | - |
...
| `deploy.logging.console.format` | Specifies the console log format. | `colored` | Valid options: `colored` (default colored text format, human-readable), `json` (JSON format with custom fields: time, service, namespace, pod), `ecs` (Elastic Common Schema format for Elasticsearch/ELK stack). Useful for log aggregation pipelines. |
| `deploy.logging.console.json` | **DEPRECATED:** Use `deploy.logging.console.format` instead. If set to `true`, format will be `json`. | `false` | The `POD_NAME` and `POD_NAMESPACE` environment variables are automatically injected via the Downward API and used to populate the `pod` and `namespace` fields. |
